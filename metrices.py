...
class measures(object):
    ...
    def __init__(self, actual, predicted, loc, labels=[0, 1]):
        ...
        self.dframe = pd.DataFrame(list(zip(self.actual, self.predicted, self.loc)),
                                   columns=['Actual', 'Predicted', 'LOC'])
    ...
    def get_performance(self):
        ...
        acc = round(1.0 * (self.tp + self.tn) / (self.tp + self.tn + self.fp + self.fn), 2) if (
                                                                                                           self.tp + self.tn + self.fp + self.fn) != 0 else 0
        f1 = round(2.0 * self.tp / (2.0 * self.tp + self.fp + self.fn), 2) if (
                                                                                          2.0 * self.tp + self.fp + self.fn) != 0 else 0
